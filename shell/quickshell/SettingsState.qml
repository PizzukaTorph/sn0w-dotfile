...
Item {
    ...
    function save(): void {
        ...
        saveProc.command = [
            "python3",
            "-c",
            "import json,os,sys; p=os.path.expanduser('~/.config/sn0w/settings.json'); os.makedirs(os.path.dirname(p),exist_ok=True); json.dump(json.loads(sys.argv[1]),open(p,'w'),indent=2); print(p)",
            JSON.stringify(data)
        ]
